...
|--------|-------|--------|--------|
| Create conversation (new top-level conversation) | Allow | Allow | Allow |
| Delete conversation | Allow | Deny | Deny |
| Rename conversation (`PATCH` title) | Allow | Allow | Deny |
| Pin / unpin conversation | Allow | Allow | Deny |
| Add member | Allow | Deny | Deny |
...
| Append **private** graph event (`visible_to` = self) | Allow | Allow | Deny |
| Commit private branch (promote private subtree to shared) | Allow | Allow | Deny |
| Delete private draft subtree | Allow | Allow | Deny |
...
|---------|----------------|
| Role value (`owner` \| `editor` \| `viewer`) | **Database** `CHECK` on `conversation_members.role` |
